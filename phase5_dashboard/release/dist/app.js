(function () {
  ...
  function input() {
    ...
    const frame=replayFrameAt(replayTime);
    const samples=[];for(let seconds=0;seconds<=10;seconds+=.5)samples.push(replayFrameAt(Math.max(0,replayTime-seconds)));
  ...
  function installReplay(value) {
    ...
    for(const id of ['trackOutline','trackPath','trackDash'])$(id).setAttribute('d',T.path(replay.points));
    $('zones').style.display=replay.hasLocation?'none':'';
    $('trackTitle').textContent=replay.source==='historical'?replay.name:(replay.source==='imported'?'Imported session':'Strategy circuit');
    $('replayBadge').textContent=replay.source==='historical'?'HISTORICAL REFERENCE':(replay.source==='imported'?'IMPORTED REPLAY':'SYNTHETIC');
    $('dataBadge').textContent=replay.source==='historical'?'Observed telemetry / simulated energy and branches':(replay.source==='imported'?'Imported replay / modelled race state':'Synthetic replay / modelled energy');
    $('towerLap').textContent=(replay.source==='imported'?'MODEL L':'LAP ')+state.lap;
    $('circuitNote').textContent=replay.source==='historical'?'Observed FastF1 positions / simulated branch outcomes':(replay.hasLocation?'Imported path / modelled rivals':'Schematic circuit / simulated car positions');
  ...
  function installHistorical(value) {
    ...
    scenario={...scenario,id:'historical-2019-abu-dhabi',name:value.race.event+' / 2019',caption:'Observed telemetry reference; branch futures are simulated.',state:{...state}};
  ...
  function drawFrame() {
    ...
    if(replay.source==='synthetic'&&streamedFrame&&streamSoc===state.soc&&Math.abs(streamedFrame.t-replayTime)<.35){frame={...frame,speed:streamedFrame.speed,gear:streamedFrame.gear,throttle:streamedFrame.throttle,brake:streamedFrame.brake,soc:streamedFrame.soc};}
    ...
    else{place('carAhead',T.point(frame.progress+Math.max(.018,state.gapAhead/90)));place('carRear',T.point(frame.progress-Math.max(.018,state.gapBehind/90)));}
    ...
    $('sectorLabel').textContent=replay.source==='imported'?'RECORDED SAMPLES':'SECTOR '+Math.min(3,1+Math.floor(frame.progress*3));
    $('replayTime').textContent=String(Math.floor(replayTime/60)).padStart(2,'0')+':'+String(Math.floor(replayTime%60)).padStart(2,'0');
    $('scrubber').value=String(Math.round(replayTime*10));
    const values=[];for(let i=0;i<32;i++){const f=T.at(replay,Math.max(0,replayTime-8+i/4));values.push((i*160/31).toFixed(1)+','+(24-f.speed/450*24).toFixed(1));}
  ...
  function recalculate() {
    try{
      ...
      $('aheadPosition').textContent=String(state.position-1).padStart(2,'0');$('yourPosition').textContent=String(state.position).padStart(2,'0');$('behindPosition').textContent=String(state.position+1).padStart(2,'0');$('towerLap').textContent=(replay.source==='imported'?'MODEL L':'LAP ')+state.lap;
  ...
  function renderRecommendation(){
    ...
    $('modelStatus').textContent=o.feasible?(window.ApexModel?'DATA MODEL + CHECKS PASS':'MODEL CHECKS PASS'):'RECOVERY REQUIRED';$('modelStatus').className='pill '+(o.feasible?'good':'warning');
    renderGnn();
  ...
  function renderLab(){
    ...
    $('constraints').innerHTML=E.ACTIONS.map(a=>{const v=E.energy(state,a,scenario.recovery[0]);return '<div class="constraint-row"><b>'+a+'</b><span class="pill '+(v.legal?'good':'warning')+'">'+(v.legal?'PASS':'BLOCKED')+'</span><span>'+esc(v.legal?fixed(v.deployMJ)+' MJ deploy / '+fixed(v.harvestMJ)+' MJ recovery':v.violations.join('; '))+'</span></div>';}).join('');
  }
  ...
  $('telemetryFile').addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;$('importStatus').textContent='Reading telemetry...';try{if(file.size>20*1024*1024)throw new Error('Keep the telemetry file below 20 MB.');const imported=T.parse(JSON.parse(await file.text()),file.name);installReplay(imported);invalidate();$('importStatus').textContent=imported.frames.length+' samples loaded. Rival gaps and energy are still simulated.';toast('Telemetry imported. Recorded fields are labelled.');}catch(err){$('importStatus').textContent='Import failed: '+err.message;}finally{e.target.value='';}});
  $('syntheticButton').addEventListener('click',()=>{installReplay(T.synthetic(state.soc));invalidate();$('importStatus').textContent='Bundled synthetic replay restored.';});
