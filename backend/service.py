...
from .gnn_adapter import FrozenGNNAdapter
...
class Service:
    def __init__(self, database=None):
        ...
        # The frozen adapter verifies its manifest and loads the selected
        # checkpoint once. If PyTorch/PyG is unavailable, status is explicit
        # and the no-ML application remains usable.
        self.gnn = FrozenGNNAdapter(os.environ.get("APEX_GNN_DEVICE", "auto"))
        self.gnn_status = self.gnn.manifest_status()
        self.phase5 = Phase5Service(gnn=self.gnn)
    ...
    def health(self, framework="stdlib"):
        return {"status": "ok", "engine_version": self.metadata["version"], "framework": framework, "storage": "sqlite", "offline": True, "websocket": framework == "fastapi", "gnn": self.gnn.manifest_status()}
