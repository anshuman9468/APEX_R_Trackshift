...
class ModelPredictRequest(BaseModel):
    """Raw graph request emitted by the audited canonical graph builder."""

    model_config = ConfigDict(extra="forbid")
    request_id: str | None = Field(default=None, min_length=8, max_length=80, pattern=r"^[a-zA-Z0-9-]+$")
    decision_timestamp_session_sec: float
    attacker_driver: str = Field(min_length=1, max_length=20)
    target_driver_fixed: str = Field(min_length=1, max_length=20)
    input_freshness: dict
    graph: dict
...
def model_status(request: Request):
    return request.app.state.service.gnn.manifest_status()


@app.post("/api/model/predict")
def model_predict(body: ModelPredictRequest, request: Request):
    result = request.app.state.service.gnn.predict_request(body.model_dump())
...
@app.get("/api/phase5/historical-state/{window_id}")
def phase5_historical_state(window_id: str, request: Request):
    """Return one audited timestamped replay state; never synthesize gaps."""
    return request.app.state.service.phase5.historical_state(window_id)
